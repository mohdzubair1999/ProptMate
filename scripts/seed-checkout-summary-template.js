// Creates the "Check-out summary (simple) — Schedule of condition" template.
// Run with: node --env-file=.env scripts/seed-checkout-summary-template.js YOUR_COMPANY_ID
...
const sections = [
  ...
  },
  {
    title: "Keys",
    fields: [
      { label: "Keys returned", type: "INFO_TEXT", options: "List all keys being returned by the tenant — quantity, key type and location." },
      { label: "Keys received from tenant(s)", type: "TEXT" },
  ...
  },
  {
    title: "Follow-up",
    fields: [
      { label: "Follow-up — cleaning", type: "TEXT" },
      { label: "Cleaning photos", type: "PHOTO" },
      { label: "Follow-up — maintenance", type: "TEXT" },
      { label: "Maintenance photos", type: "PHOTO" },
      { label: "— Page break —", type: "PAGE_BREAK" },
    ],
  },
  {
    title: "Terms and conditions",
    fields: [{ label: "Disclaimer", type: "TERMS", options: "This check-out report reflects the property's condition at the time of inspection." }],
  },
  {
    title: "Declaration",
    fields: [
...
async function main() {
  ...
  if (!companyId) {
    console.error("Usage: node --env-file=.env scripts/seed-checkout-summary-template.js YOUR_COMPANY_ID");
    process.exit(1);
  }
  ...
  const template = await prisma.template.create({
    data: { companyId, name: "Check-out summary (simple)", inspectionType: "check-out", propertyType: null },
  });
