// Creates the "Mid Term: Studio" template — reconstructed and redesigned.
// Run with: node --env-file=.env scripts/seed-midterm-studio-template.js YOUR_COMPANY_ID
...
const rooms = ["Communal hallway", "Studio room", "Kitchen area of studio", "Bathroom area in studio"];
...
async function main() {
  ...
  if (!companyId) {
    console.error("Usage: node --env-file=.env scripts/seed-midterm-studio-template.js YOUR_COMPANY_ID");
    process.exit(1);
  }
  ...
  const template = await prisma.template.create({
    data: { companyId, name: "Mid Term: Studio", inspectionType: "mid-term", propertyType: "studio" },
  });
