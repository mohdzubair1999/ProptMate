// Creates the "Mid Term: Inspection Report" template — reconstructed and redesigned.
// Run with: node --env-file=.env scripts/seed-midterm-inspection-report-template.js YOUR_COMPANY_ID
...
async function main() {
  ...
  if (!companyId) {
    console.error("Usage: node --env-file=.env scripts/seed-midterm-inspection-report-template.js YOUR_COMPANY_ID");
    process.exit(1);
  }
  ...
  const template = await prisma.template.create({
    data: { companyId, name: "Mid Term: Inspection Report", inspectionType: "mid-term", propertyType: "hmo" },
  });
