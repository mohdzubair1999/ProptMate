...
const RULES: RoomItemRule[] = [
  ...
  // Checked before the plain "kitchen" rule below on purpose - an open-plan title like
  // "Open Plan Kitchen/Lounge" also satisfies that rule's own /kitchen/i test, and
  // RULES.find() stops at the first match, so this more specific combination would never
  // actually be reached if it came after the generic kitchen rule instead of before it.
  {
    matches: (t) => /kitchen/i.test(t) && /lounge|living|reception/i.test(t),
    items: [
      "Oven / cooker",
      "Hob",
      "Fridge / freezer",
