...
// Ready-made inventory sections for room layouts common enough to be worth a one-click add,
// rather than building the section and its inventory field manually every time. Each is a
// TemplateSection with a single INVENTORY_SECTION field - the actual starter item
// suggestions for items added within it come from lib/standardInventoryItems.ts, matched by
// this section's own title, so the two stay in sync automatically without needing to repeat
// the item list here too.
const QUICK_INVENTORY_SECTIONS: Record<string, { title: string; fieldLabel: string }> = {
  "open-plan-kitchen-lounge": { title: "Open Plan Kitchen/Lounge", fieldLabel: "Open Plan Kitchen/Lounge" },
};

export async function addQuickInventorySection(formData: FormData) {
  const companyId = await requireManagerOrAdmin();
  const templateId = String(formData.get("templateId") || "");
  ...
  const preset = QUICK_INVENTORY_SECTIONS[sectionKey];
  if (!preset) throw new Error("Unknown quick-add section");
  ...
  const createdSection = await prisma.templateSection.create({
    data: { templateId, title: preset.title, order: existingCount },
  });
  await prisma.templateField.create({
    data: { sectionId: createdSection.id, label: preset.fieldLabel, type: "INVENTORY_SECTION", order: 0 },
  });
