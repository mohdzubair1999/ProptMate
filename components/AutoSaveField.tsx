...
export default function AutoSaveField({ inspectionId, fieldId, type, initialValue, options = [], id }: Props) {
  ...
  const doSave = (v: string) => {
    ...
    saveFieldValue(inspectionId, fieldId, v)
      ...
      .catch(async () => {
        // The most common real cause of a failed save here is losing signal mid-inspection
        // — the previous behaviour just reverted to idle and silently lost the change. Now
        // it's queued locally and synced automatically once connection returns, rather than
        // just disappearing.
        await queueWrite({ type: "saveFieldValue", args: [inspectionId, fieldId, v] });
        setStatus("queued");
      });
  };
  ...
  const indicator =
    status === "saving" ? (
      ...
      <span className="text-xs text-signal">Saved locally — will sync</span>
    ) : null;
