...
import { updateInventoryItem, deleteInventoryItem, deletePhoto } from "@/lib/actions/inspections";
      ...
      <div className="border border-line rounded-lg p-3 flex items-start justify-between">
        <div className="flex-1">
          ...
          {isDraft && item.photos.length > 0 && inspectionId && (
            <AnalyzePhotoButton photoUrls={item.photos.map((p) => p.url)} itemId={item.id} inspectionId={inspectionId} context={`${item.room ? item.room + " — " : ""}${item.itemName}`} />
          )}
