...
import SectionTabsLayout, { type SectionTab } from "./section-tabs-layout";
...
export default async function InspectionDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ justCompleted?: string }> }) {
  ...
  const siblingInspections = await prisma.inspection.findMany({
    where: { propertyId: inspection.propertyId, id: { not: inspection.id }, status: "completed", deletedAt: null },
    orderBy: { completedDate: "desc" },
  });

  // Everyone already linked to this property via the tenant/landlord portal invite feature —
  // used to pre-fill the email form below, so sending a finished report to a property with
  // several tenants doesn't mean retyping every address by hand each time.
  const linkedPropertyContacts = await prisma.propertyAccess.findMany({
    where: { propertyId: inspection.propertyId },
    include: { user: { select: { email: true, name: true } } },
  });
  const linkedContactsWithNames = linkedPropertyContacts.map((a) => ({ email: a.user.email, name: a.user.name || "" }));

  const existingAcknowledgements = await prisma.reportAcknowledgement.findMany({
  ...
  // The comparison panel only ever applies to template-based inspections (its own condition
  // already required inspection.templateId before this change too), so this — together with
  // the summary/reference fields — becomes the "Overview" tab whenever there's a template.
  const overviewContent = (
    <>
      ...
      {(siblingInspections.length > 0 || inspection.comparedToInspection) && isDraft && inspection.templateId && (
        <section className="mt-6 bg-white border border-line rounded-xl p-4">
            ...
            <>
              ...
              <ComparisonSelect
                ...
                options={siblingInspections.map((s) => ({
                  id: s.id,
                  label: `${s.type.replace("-", " ")} — ${s.completedDate ? formatDate(s.completedDate) : "no date"}`,
          ...
          {inspection.comparedToInspection && !inspection.comparedToInspection.sourceDocumentUrl && (
            <p className="text-xs text-verified mt-2">
              ...
              {inspection.comparedToInspection.completedDate ? formatDate(inspection.comparedToInspection.completedDate) : "—"}.
            </p>
          )}
  ...
  const roomTabs: SectionTab[] = inspection.templateId
    ? await TemplateInspectionView({
        inspectionId: inspection.id,
  ...
  const allTabs: SectionTab[] = [{ id: "overview", title: "Overview", content: overviewContent }, ...roomTabs];
    ...
    <main>
      ...
      <div className="flex items-start justify-between mt-4 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          {isDraft && inspection.templateId && templateFieldsForImport.length > 0 && (!inspection.comparedToInspection || inspection.comparedToInspection.sourceDocumentUrl) && (
            <MenuModal label="Upload check-in report" icon="📄" title="Upload external check-in report">
          ...
          {canManage && (
            <form action={deleteInspection}>
              <input type="hidden" name="inspectionId" value={inspection.id} />
              <ConfirmSubmitButton
                confirmMessage="Delete this inspection? It'll move to Recently Deleted (Settings), where it can be restored at any time."
      ...
      {inspection.templateId ? (
        <SectionTabsLayout sections={allTabs} />
      ) : (
        <>
          {overviewContent}

          {isDraft && inspection.items.length > 0 && (
            <VoiceNoteRecorder inspectionId={inspection.id} items={inspection.items.map((i) => ({ id: i.id, room: i.room, itemName: i.itemName }))} />
          )}
          ...
          {inspection.items.length === 0 ? (
            ...
            <div className="mt-4 space-y-2">
              {inspection.items.map((item) => (
                <EditableInventoryItem key={item.id} item={item} isDraft={isDraft} inspectionId={inspection.id} inspectionType={inspection.type} />
              ))}
            </div>
          )}
          ...
          <section className="mt-6 bg-white border border-line rounded-xl p-6">
            ...
            {inspection.report?.pdfUrl ? (
              <div className="mt-3">
                ...
                <SendReportForm
                  inspectionId={inspection.id}
                  initialRecipients={linkedContactsWithNames}
                  defaultSubject={`${inspectionTypeDisplayName(inspection.type)} — ${inspection.property.address}`}
                  reportFileName={`${(inspection.template?.name || `${inspection.type} inspection report`).replace(/[^a-z0-9]+/gi, "-")}.pdf`}
                />

                {existingAcknowledgements.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-line space-y-1.5">
                    <p className="text-xs text-slate mb-1">Confirmation status:</p>
                    {existingAcknowledgements.map((ack) => (
                      <div key={ack.id} className="flex items-center justify-between text-sm bg-paper rounded-lg px-3 py-2">
                        <span className="text-ink">
                          {ack.recipientName} <span className="text-slate">· {ack.recipientEmail}</span>
                        </span>
                        {ack.confirmedAt ? (
                          <span className="text-xs text-verified">✓ Confirmed {formatDate(ack.confirmedAt)}</span>
                        ) : (
                          <span className="text-xs text-signal">⏳ Sent {formatDate(ack.sentAt)}, awaiting confirmation</span>
                        )}
                      </div>
                    ))}
                  </div>
