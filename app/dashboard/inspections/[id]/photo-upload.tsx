...
import { queuePhoto } from "@/lib/offlinePhotoQueue";
import { isGenuinelyOnline } from "@/lib/connectivity";
...
export default function PhotoUpload({ itemId, fieldId, inspectionId }: { itemId?: string; fieldId?: string; inspectionId?: string }) {
  ...
  const uploadFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      // Checked before attempting anything - queuing immediately when offline avoids a
      // network call that can only fail ambiguously, the same pattern already established
      // for the offline queue (see lib/offlineQueue.ts and lib/offlinePhotoQueue.ts).
      if (!(await isGenuinelyOnline())) {
        await queueLocally();
      ...
      try {
        ...
        // The connectivity check above passed, but the actual request still failed - could
        // still be a genuine, real connectivity drop in the brief window since that check,
        // so re-confirm before deciding this was a real failure rather than losing the photo.
        if (!(await isGenuinelyOnline())) {
          await queueLocally();
        } else {
          failures++;
    ...
    const uploadedNow = files.length - failures - queued;
    if (uploadedNow > 0) router.refresh(); // nothing new server-side to fetch otherwise
  };
    ...
    <div className="mt-2">
      ...
      {queuedCount > 0 && (
        <p className="text-xs text-amber-700 mt-1">
          📴 {queuedCount === 1 ? "1 photo" : `${queuedCount} photos`} saved on this device — will upload automatically once you're back online.
        </p>
      )}
    </div>
  );
