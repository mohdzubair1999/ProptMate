    ...
    <main>
      ...
      {totalCount === 0 ? (
        <section className="mt-6 bg-white border border-line rounded-xl p-10 text-center">
          <p className="text-slate text-sm">No inspections yet.</p>
        </section>
      ) : inspections.length === 0 ? (
        <section className="mt-6 bg-white border border-line rounded-xl p-10 text-center">
          <p className="text-slate text-sm">No inspections match your search.</p>
        </section>
      ) : (
        <div className="mt-6 bg-white border border-line rounded-xl overflow-hidden">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {inspections.map((i) => (
                <tr key={i.id} className="relative border-b border-line last:border-0 hover:bg-paper transition-colors">
                  <td className="px-6 py-4">
                    <Link href={`/dashboard/inspections/${i.id}`} className="text-ink font-medium hover:text-signal after:absolute after:inset-0">
                      {i.property.address}
                    </Link>
                  </td>
