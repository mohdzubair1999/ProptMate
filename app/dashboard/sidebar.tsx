  ...
  return (
    <aside className="hidden md:flex w-60 shrink-0 bg-ink text-white flex-col justify-between min-h-screen sticky top-0">
      ...
      <div className="px-6 py-6 border-t border-white/10">
        ...
        <Link href="/dashboard/profile" className="block mt-3 text-sm text-white/70 hover:text-white transition-colors">
          My profile
        </Link>
        <button onClick={() => signOut({ fetchOptions: { onSuccess: () => router.push("/") } })} className="mt-2 text-sm text-white/70 hover:text-white transition-colors">
          Log out
        </button>
      </div>
    </aside>
