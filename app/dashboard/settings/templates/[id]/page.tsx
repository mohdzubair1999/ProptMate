...
import { addSection, addRegulationBlock, addQuickInventorySection, unhideSection } from "@/lib/actions/templates";
...
export default async function TemplateEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="max-w-3xl">
      ...
      {canManage && (
        <section className="mt-6 bg-white border border-line rounded-xl p-6">
          <h2 className="font-display font-600 text-ink mb-1">Quick-add an inventory section</h2>
          <p className="text-sm text-slate mb-4">Ready-made sections for common room layouts, pre-set with sensible starter items to check off.</p>
          <div className="flex flex-wrap gap-2">
            <form action={addQuickInventorySection}>
              <input type="hidden" name="templateId" value={template.id} />
              <input type="hidden" name="sectionKey" value="open-plan-kitchen-lounge" />
              <button type="submit" className="text-sm px-4 py-2 rounded-full border border-line text-ink hover:border-signal hover:text-signal transition-colors">
