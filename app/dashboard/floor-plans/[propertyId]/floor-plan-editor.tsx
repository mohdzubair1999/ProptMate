...
// whichever shape it is, plus the "footprint" height actually needed for layout purposes —
// a bay window protrudes past lengthM, so packing needs to reserve that extra depth or the
// bay would visually overlap whatever room ends up below it.
...
function computeLayout(
  ...
  const withGeometry = rooms.map((r) => ({ ...r, geometry: roomGeometry(r) }));
  const totalArea = withGeometry.reduce((sum, r) => sum + r.widthM * r.geometry.footprintHeight, 0);
  const targetWidth = Math.max(Math.sqrt(totalArea) * 1.3, Math.max(...withGeometry.map((r) => r.widthM)));
  ...
  const positioned = withGeometry.map((room) => {
    if (currentX > 0 && currentX + room.widthM > targetWidth) {
      currentX = 0;
      currentY += rowHeight;
      rowHeight = 0;
    }
    ...
    currentX += room.widthM;
    rowHeight = Math.max(rowHeight, room.geometry.footprintHeight);
  ...
  const totalWidth = Math.max(...positioned.map((r) => r.x + r.widthM), 0);
  const totalHeight = Math.max(...positioned.map((r) => r.y + r.geometry.footprintHeight), 0);

  return { positioned, totalWidth, totalHeight };
}

// Bounding-box overlap check — a reasonable, honest approximation rather than true
// polygon-vs-polygon intersection, which would be genuinely more complex to get right for
// bay windows and L-shapes. This can occasionally flag two irregular rooms as "overlapping"
// when their exact carved-out shapes wouldn't actually touch — but erring toward flagging a
// possible overlap is safer than silently missing a real one.
function detectCollisions(positioned: { i: number; x: number; y: number; widthM: number; geometry: { footprintHeight: number } }[]): Set<number> {
  const overlapping = new Set<number>();
  for (let a = 0; a < positioned.length; a++) {
    for (let b = a + 1; b < positioned.length; b++) {
      const r1 = positioned[a];
      const r2 = positioned[b];
      const overlaps = r1.x < r2.x + r2.widthM && r1.x + r1.widthM > r2.x && r1.y < r2.y + r2.geometry.footprintHeight && r1.y + r1.geometry.footprintHeight > r2.y;
      if (overlaps) {
        overlapping.add(r1.i);
        overlapping.add(r2.i);
      }
    }
  }
  return overlapping;
...
export default function FloorPlanEditor({ propertyId, initialLevels }: { propertyId: string; initialLevels: Level[] }) {
  ...
  const generateRoomsFromImage = async (file: File): Promise<{ rooms: Room[]; dimensionsFromSketch: boolean } | { error: string }> => {
    try {
      ...
      const rooms: Room[] = data.rooms.map((r: any) => ({
        ...
        shape: r.shape === "bay-window" || r.shape === "l-shape" ? r.shape : "rectangle",
  ...
  const resizeDragRef = useRef<{ roomIndex: number; axis: "width" | "length"; lastClientX: number; lastClientY: number; rawValue: number } | null>(null);

  const handleResizePointerDown = (e: React.PointerEvent, roomIndex: number, axis: "width" | "length", currentValue: number) => {
    e.stopPropagation(); // otherwise the room's own drag handler on the parent <g> fires too
    resizeDragRef.current = { roomIndex, axis, lastClientX: e.clientX, lastClientY: e.clientY, rawValue: currentValue };
    (e.target as Element).setPointerCapture(e.pointerId);
    isDraggingRef.current = true;
    dragHasCommittedRef.current = false;
  };
  ...
  const handleResizePointerMove = (e: React.PointerEvent) => {
    ...
    e.stopPropagation();
    const { roomIndex, axis, lastClientX, lastClientY, rawValue } = resizeDragRef.current;
    const deltaM = axis === "width" ? (e.clientX - lastClientX) / PIXELS_PER_METRE : (e.clientY - lastClientY) / PIXELS_PER_METRE;
    const nextRawValue = rawValue + deltaM;
    resizeDragRef.current = { roomIndex, axis, lastClientX: e.clientX, lastClientY: e.clientY, rawValue: nextRawValue };
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      updateRoom(
                        ...
                        newShape === "rectangle"
                          ? { shape: newShape }
                          : { shape: newShape, isStairs: false, stairDirection: undefined, stairLinkFloor: undefined, stairLinkRoom: undefined }
                      );
                    }}
                      ...
                      {resizeMode && (
                        <>
                          <circle
                            ...
                            onPointerDown={(e) => handleResizePointerDown(e, room.i, "width", room.widthM)}
                          ...
                          <circle
                            ...
                            onPointerDown={(e) => handleResizePointerDown(e, room.i, "length", room.lengthM)}
                            onPointerMove={handleResizePointerMove}
                            onPointerUp={handleResizePointerUp}
                          />
                        </>
