...
const features = [
  {
    icon: "⚖️",
    title: "AI dispute-risk scoring",
    detail: "Every item compared check-in to check-out and classified as fair wear and tear, tenant responsibility, or genuinely unclear — before a deposit dispute ever gets filed.",
  },
  {
    icon: "🎙️",
    title: "Voice-narrated inspections",
    detail: "Speak as you walk the property. AI routes what you say to the right room and item automatically, and flags anything it isn't confident about for you to confirm.",
  },
  {
    icon: "📸",
    title: "AI photo analysis",
    detail: "Upload photos and get an accurate, hedged condition description back — vinyl vs tile, superficial vs structural — with the AI asking you directly when something's genuinely ambiguous.",
  },
  {
    icon: "📐",
    title: "To-scale floor plans",
    detail: "Real measurements, real angles, custom wall shapes for bay windows and alcoves — drawn to actual scale, not a rough sketch.",
  },
  {
    icon: "🛡️",
    title: "Real-time security monitoring",
    detail: "Every property protected by live threat detection, automatic rate limiting, and instant alerts — the same infrastructure a much larger platform would run.",
  },
  {
    icon: "📄",
    title: "Branded PDF reports",
    detail: "Every report leaves with your agency's own branding, signed, dated, and ready to send before you've left the driveway.",
  },
];
    ...
    <main className="min-h-screen bg-paper">
      ...
      <section className="px-6 sm:px-10 pb-24 max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="font-display font-700 text-2xl sm:text-3xl text-ink">Built for how inspections actually go</h2>
          <p className="mt-3 text-slate max-w-xl mx-auto">Not a generic checklist app — purpose-built AI and tooling for the specific problems a real inventory clerk runs into.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-line rounded-3xl overflow-hidden shadow-sm">
          {features.map((feature) => (
            <div key={feature.title} className="bg-white p-6 sm:p-8">
              <span className="text-2xl" aria-hidden="true">
                {feature.icon}
              </span>
              <h3 className="font-display font-600 text-lg text-ink mt-3">{feature.title}</h3>
              <p className="text-sm text-slate mt-2">{feature.detail}</p>
            </div>
          ))}
        </div>
      </section>
