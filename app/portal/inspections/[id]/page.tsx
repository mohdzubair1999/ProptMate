...
export default async function PortalInspectionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const inspection = await prisma.inspection.findFirst({
    where: { id, assignedClientId: userId, deletedAt: null },
  ...
  const roomSections = inspection.templateId
    ? await TemplateInspectionView({ inspectionId: inspection.id, templateId: inspection.templateId, isDraft })
    : [];
