...
import { getMainAdminEmail } from "@/lib/getMainAdminEmail";
...
export async function POST(req: Request) {
  ...
  const { inspectionId, recipientEmails, message, subject: customSubject } = await req.json();
  ...
  const replyToEmail = await getMainAdminEmail(companyId);
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: fromAddress,
        to: recipientEmails,
        subject: typeof customSubject === "string" && customSubject.trim() ? customSubject.trim() : `${displayName} — ${inspection.property.address}`,
