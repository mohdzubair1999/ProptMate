...
export async function POST(req: Request) {
  ...
  const systemPrompt =
    ...
    "IMPORTANT: one or more input descriptions may contain a marker in this exact format: [CLARIFY: a question]. Never paraphrase, merge, summarise, or reword these markers - copy each one through into your combined result completely unchanged, character for character, as its own separate sentence (never embedded in the middle of another sentence, since it will be removed from the final text and the surrounding sentence needs to still read correctly without it). If the same question genuinely appears more than once, keep only one copy of it; if different [CLARIFY] markers ask genuinely different questions, keep all of them. " +
